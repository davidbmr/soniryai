...
const genres = [
    "Jazz", "Rock", "Funk", "Hip hop", "EDM", "Salsa",
    "Metal", "Cumbia", "Reggae", "Reggaeton", "Clásica", "Alternative"
];
            ...
            <div className={style.section__container}>
                ...
                <div className={style.rangeFields__container}>
                    <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
                        ...
                        <RangeFieldX name="ritmo" firstText={"¿Cuál es el ritmo de su corazón?"} initialValue={50} onValueChange={(value: number) => handleRangeChange("ritmo", value)} />
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
                        <BsSoundwave fill="#fff" size={50} />
                        <RangeFieldX name="tono" firstText={"¿Cuál es su tono de voz?"} initialValue={50} onValueChange={(value: number) => handleRangeChange("tono", value)} />
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
                        <BsSoundwave fill="#fff" size={50} />
                        <RangeFieldX name="fuerza" firstText={"¿Qué tan fuerte suena?"} initialValue={50} onValueChange={(value: number) => handleRangeChange("fuerza", value)} />
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
                        <BsSoundwave fill="#fff" size={50} />
                        <RangeFieldX name="estilo" firstText={"¿Clásico o moderno?"} initialValue={50} onValueChange={(value: number) => handleRangeChange("estilo", value)} />
                    </div>
                </div>
                <br />
                ...
                <div>
                    ...
                    <div className={style.box__sounds}>
                        {genres.map((genre) => (
                            <div
                                key={genre}
                                className={`${style.box__sound__item} ${selectedGenres.includes(genre) ? style.active : ""}`}
